...
resize_for_tilemap :: proc(tm: Tilemap, eng: ^engine.Engine_Context) {
	engine.resize(eng, i32(tm.width * CELL_SIZE), i32(tm.height * CELL_SIZE) + HUD_HEIGHT)
}
...
draw_tilemap :: proc(tm: Tilemap, assets: Assets, remaining: int) {
	...
	t = t - f32(int(t / 10000)) * 10000
	if assets.sprites.grass_shader_valid {
		if assets.sprites.grass_time_loc >= 0 {
			raylib.SetShaderValue(
				assets.sprites.grass_shader,
				assets.sprites.grass_time_loc,
				&t,
				.FLOAT,
			)
		}
